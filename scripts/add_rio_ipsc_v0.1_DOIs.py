...
def _ingest_DOI_doc(
    ...
    for indiv in data:
        ...
        if affiliation == "":
            ...
            if "\n" in affiliation:
                affiliation = [
                    x.strip().rstrip(";").rstrip(".") for x in affiliation.split("\n") if x.strip() != ""
                ]
...
for dataset in datasets:
    ...
    intake_doc = ds_path / f"refs/{dataset}_Info.docx"
    ...
    # INGEST DOI DOCS
    _setup_DOI_info(ds_path, intake_doc, publication_date="2026-04-15")

# %%



# now generate teh v0.1 datasets
...
for dataset in datasets:
    ...
    # deposition, metadata = create_draft_doi(zenodo, ds_path, version=current_dataset_version, metadata=local_metadata)

    # deposition = zenodo.create_new_deposition(metadata)
    ...
    deposition = zenodo.change_metadata(local_metadata)
...
#     local_metadata = create_draft_metadata(ds_path, version="1.1")
#     metadata = local_metadata
#     metadata['version'] = '1.1'

#     deposition = update_doi_metadata(zenodo, new_doi_id, metadata)

#########. Warning Need to update grants by hand...

# %%

########################
# have not executed below....
# %%
zenodo = setup_zenodo()
...
# %%
###########
##. need to fix the grants by hand....abs
